...
export default function({isLoginPage}:any){
    ...
    return(
        <nav className={`flex border ${theme?'border-blackTheme':'border-whiteTheme'} h-14`}>   
            ...
            {(isLoginPage)?<div className="absolute right-4 mt-2"><Button variant={`${(theme)?"secondary":"default"}`}>Login</Button></div>:<div className={`rounded-full w-11 h-11 ${theme?'bg-secondary text-primary':'bg-primary text-secondary'} absolute right-4 mt-1 text-lg pt-2 pl-3.5 cursor-pointer font-bold`}>
                P
            </div>}
        </nav>
    )
