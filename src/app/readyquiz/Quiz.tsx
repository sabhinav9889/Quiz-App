...
export default function Quiz() {
  const { theme, setTheme, newQuiz, setnewQuiz, questionSet, setQuestionSet } =
    useContext(messageData)!;
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      let quesData = {
        topic: topic,
        numberOfQuestions: Number(ques),
        type: tab ? "MCQ" : "Open Ended",
        score: 0,
      };
      if (ques) {
        setnewQuiz(quesData);
        const res = await axios.post("/api/getNewQuiz", quesData);
  ...
  return (
    <div
      className={`${
        theme ? "bg-blackBg text-white" : "bg-white text-black"
      } min-h-screen`}
    >
      <Nav isLoginPage={false} />
      <form onSubmit={handleSubmit}>
        <div
          className={`sm:pr-14 sm:pl-14 pr-4 pl-4 mt-14 border xl:mr-80 xl:ml-80 rounded-lg md:ml-40 md:mr-40 sm:ml-20 sm:mr-20 ml-3 mr-3 hover:shadow-lg ${
            theme ? "hover:shadow-destructive" : ""
          } shadow-sm`}
        >
          <div className={`w-full rounded-md`}>
            <p className="w-full flex justify-center mt-4 p-4 font-bold text-lg">
              Quizify
            </p>
            ...
            <div className="w-full mt-1 flex justify-center">
              <input
                ...
                className={`${
                  theme ? "bg-blackBg text-white" : "bg-white text-black"
                } w-full p-4 h-10 border rounded-md`}
            ...
            <div className="w-full flex mt-3 bg-destructive rounded-lg p-3 font-bold">
              <div
                className={`w-1/2 text-center ${
                  tab ? "bg-blackBg text-white" : "bg-destructive"
